...
const program = Effect.gen(function* () {
  const input = yield* ProblemInput.read("input.txt", "day8");
  ...
  while (true) {
    ...
    if (Graph.connectedComponents(circuits).length === 1) {
      const posA = Graph.getNode(graph, minEdge[1]!.source).pipe(
        Option.getOrThrow,
      );
      const posB = Graph.getNode(graph, minEdge[1]!.target).pipe(
        Option.getOrThrow,
      );
      xMul = posA.x * posB.x;
      break;
